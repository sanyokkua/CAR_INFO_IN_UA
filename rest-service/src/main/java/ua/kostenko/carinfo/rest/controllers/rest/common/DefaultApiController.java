...
@Slf4j
public abstract class DefaultApiController<T extends GenericRecord<I>, I, D extends ResourceSupport>
        implements RestApi<T, I, D> {

    private final SearchService<T, I> searchService;
    private final Translation translation;

    protected DefaultApiController(@NonNull @Nonnull SearchService<T, I> searchService,
    ...
    private Link getCountLinkWithParams(Link link) {
        TemplateVariables tmpVars = new TemplateVariables(new TemplateVariable("indexField",
                TemplateVariable.VariableType.PATH_VARIABLE, "The main field for search"));
    ...
    protected PagedResources<D> getFindResult(PagedResourcesAssembler<T> assembler, Pageable pageable,
    ...
    private PagedResources<D> getPageableResourceFind(@NonNull @Nonnull PagedResourcesAssembler<T> assembler,
            @NonNull @Nonnull Page<T> page, String indexField) {
    ...
    protected PagedResources<D> getFindByParamsResult(PagedResourcesAssembler<T> assembler, Pageable pageable,
            T params) {
    ...
    private PagedResources<D> getPageableResource(@NonNull @Nonnull PagedResourcesAssembler<T> assembler,
            @NonNull @Nonnull Page<T> page, T params) {
        Link selfLink =
                linkTo(methodOn(getClassInstance()).findByParams(assembler, page.getPageable(), params)).withSelfRel();
        TemplateVariables tmpVars = getRequestTemplateVariables();
    ...
    private TemplateVariables getRequestTemplateVariables() {
        List<TemplateVariable> variables = getParams().stream()
                .map(param -> new TemplateVariable(param.getKey(), TemplateVariable.VariableType.REQUEST_PARAM,
                        param.getValue()))
                .collect(Collectors.toList());
        variables.add(
                new TemplateVariable("page", TemplateVariable.VariableType.REQUEST_PARAM, "Number of current page"));
